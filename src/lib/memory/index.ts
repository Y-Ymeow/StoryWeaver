...
export async function generateCharacterMemory(
  ...
  // 获取该角色的所有演出记录
  const characterPerformances = performances.filter(p => p.character_id === character.id)
  ...
  const dialogueHistory = characterPerformances
    .map(p => {
      try {
        ...
        return content.dialogue || content.action || content.thought || content.emotion || p.content;
      } catch {
        return p.content;
      }
    })
...
export async function generateSceneSummary(
  ...
  try {
    // 动态导入
    const { createClient } = await import('@/lib/openai/client');
    const { getSceneSummarySystemPrompt, buildSceneSummaryPrompt } = await import('@/lib/prompts');
    ...
    const messages = [
      ...
      { role: 'user', content: buildSceneSummaryPrompt(scene, performances, characters) }
    ];

    let summary = '';
    const stream = client.chatStream(messages, { temperature: 0.7, max_tokens: 500, model });
    for await (const chunk of stream) {
      summary += chunk;
    }

    return summary.trim() || generateBasicSummary(scene, performances, characters);
  } catch (error) {
...
function generateBasicSummary(
  ...
  if (involvedCharacters.length > 0) {
    summaryParts.push(`参与角色：${involvedCharacters.map(c => c.name).join(', ')}`);
  }
  ...
  for (const perf of performances) {
    try {
      const content = typeof perf.content === 'string' ? JSON.parse(perf.content) : perf.content;
...
export async function generateRoomSummary(
  ...
  const progress = completedScenes.length > 0 
