...
/**
 * 构建场景导演指令 Prompt - 决定下一步由谁说话、要做什么
 */
export function buildSceneDirectivePrompt(
  ...
  const charactersInfo = characters
    .map(
      (c) =>
        `- ${c.name} (${c.is_user ? "用户角色" : "AI 角色"}) [id:${c.character_id}]，背景:${c.background || "普通人"}，说话风格:${c.dialogue_style || "自然口语"}`,
    )
    .join("\n");
...
export function buildSceneSummaryPrompt(
  ...
  const performanceText = performances
    .map((p) => {
      ...
      return `[第${p.round}轮] ${char?.name || p.character_id}: ${content}`;
    })
    .join("\n");
