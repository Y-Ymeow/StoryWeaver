...
export const AIGenerate: FunctionalComponent<AIGenerateProps> = ({
  ...
  const [mode, setMode] = useState<'room' | 'character' | 'scene' | 'custom'>(presetMode)
  const [showAIInput, setShowAIInput] = useState(false)

  const handleAIResult = (result: { content: string }) => {
    const parsed = parseAIResponse(result.content, mode, roomContext, characters, scenes)
    onGenerate(parsed)
          ...
          {providers.length === 0 ? (
            ...
            <>
              <div class="flex items-center gap-2 text-sm text-gray-400">
                <span>当前使用：</span>
                <span class="text-primary-400">{providers.find(p => p.id === activeProviderId)?.name || '未设置'}</span>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">生成类型</label>
                <div class="grid grid-cols-2 gap-2">
                  <Button variant={mode === 'room' ? 'primary' : 'secondary'} onClick={() => openAIInput('room')}>🏠 房间设定</Button>
                  <Button variant={mode === 'character' ? 'primary' : 'secondary'} onClick={() => openAIInput('character')}>👥 角色</Button>
                  <Button variant={mode === 'scene' ? 'primary' : 'secondary'} onClick={() => openAIInput('scene')}>🎬 场景</Button>
                  <Button variant={mode === 'custom' ? 'primary' : 'secondary'} onClick={() => openAIInput('custom')}>📝 自定义</Button>
                </div>
              </div>
            </>
          )}
...
function parseAIResponse(content: string, mode: string, roomContext?: any, characters?: any[], scenes?: any[]): AIGenerateResult {
  try {
    ...
    if (mode === 'room') {
      ...
      return { characters: (parsed.characters || []).map((c: any) => ({ name: c.name || '', background: c.background || '', dialogue_style: c.dialogue_style || '', is_user: c.is_user || false })) }
    } else if (mode === 'scene') {
      return { scenes: (parsed.scenes || []).map((s: any) => ({ name: s.name || '', description: s.description || '', goal: s.goal || '', setup: s.setup || '', max_rounds: s.max_rounds || 10 })) }
    }
    return { content }
  } catch (e) {
